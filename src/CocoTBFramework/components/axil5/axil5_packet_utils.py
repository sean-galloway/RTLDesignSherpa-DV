...
#
# Module: axil5_packet_utils
# Purpose: AXIL5 (AXI5-Lite) Packet Utilities
#
...
Mirrors ``axil4_packet_utils``, built on ``AXIL5Packet`` so every helper can
carry the AXI5-Lite optional groups. Pass a group's width (or ``trace=True``,
``exclusive=True``) to any constructor and the field appears; omit them all and
the packet is bit-for-bit an AXI4-Lite one.
AXIL5 (AXI5-Lite) Packet Utilities

Simple utility functions for creating AXIL5 packets that use generic field names
(addr, data, resp, etc.) which match the field configuration. The pkt_prefix
parameter in GAXI components handles the AXIL5 signal mapping.
...
#: the option appearing to work while doing nothing.
...
def _split_options(kwargs):
    """Split group-enable kwargs from field values."""
    opts = {k: v for k, v in kwargs.items() if k in _OPTION_KEYS}
    values = {k: v for k, v in kwargs.items() if k not in _OPTION_KEYS}
    return opts, values



def create_simple_read_packet(address: int, prot: int = 0, **kwargs) -> AXIL5Packet:
    """
    ...
    In AXIL5, there are no burst parameters (len, size, burst_type).
    ...
    Returns:
        AXIL5Packet configured for AR channel
    """
    # Create AR field configuration
    opts, values = _split_options(kwargs)
    ar_config = AXIL5FieldConfigHelper.create_ar_field_config(**opts)

    # Create packet with AR fields
    packet = AXIL5Packet(ar_config)
...
def create_simple_write_address_packet(address: int, prot: int = 0, **kwargs) -> AXIL5Packet:
    """
    ...
    Returns:
        AXIL5Packet configured for AW channel
    """
    # Create AW field configuration
    opts, values = _split_options(kwargs)
    aw_config = AXIL5FieldConfigHelper.create_aw_field_config(**opts)

    # Create packet
    packet = AXIL5Packet(aw_config)
...
def create_simple_write_data_packet(data: int, strb: Optional[int] = None,
                                  data_width: int = 32, **kwargs) -> AXIL5Packet:
    """
    ...
    Returns:
        AXIL5Packet configured for W channel
    """
    # Create W field configuration
    opts, values = _split_options(kwargs)
    w_config = AXIL5FieldConfigHelper.create_w_field_config(data_width, **opts)

    # Create packet
    packet = AXIL5Packet(w_config)
...
def create_simple_read_response_packet(data: int, resp: int = 0,
    ...
    Returns:
        AXIL5Packet configured for R channel
    """
    # Create R field configuration
    opts, values = _split_options(kwargs)
    r_config = AXIL5FieldConfigHelper.create_r_field_config(data_width, **opts)

    # Create packet
    packet = AXIL5Packet(r_config)
...
def create_simple_write_response_packet(resp: int = 0, **kwargs) -> AXIL5Packet:
    """
    ...
    Returns:
        AXIL5Packet configured for B channel
    """
    # Create B field configuration
    opts, values = _split_options(kwargs)
    b_config = AXIL5FieldConfigHelper.create_b_field_config(**opts)

    # Create packet
    packet = AXIL5Packet(b_config)
...
def create_simple_register_write(address: int, data: int, strb: Optional[int] = None,
                               prot: int = 0, data_width: int = 32) -> Tuple[AXIL5Packet, AXIL5Packet]:
    """
...
def create_simple_register_read(address: int, prot: int = 0) -> AXIL5Packet:
    """
...
def create_register_response_packets(read_data: Optional[int] = None, write_resp: Optional[int] = None,
                                   data_width: int = 32) -> Dict[str, AXIL5Packet]:
    """
...
def validate_axil5_address_alignment(address: int, data_width: int = 32) -> Tuple[bool, str]:
    """
    Validate AXIL5 address alignment requirements.
...
def create_axil5_register_map_packets(register_map: Dict[int, int],
                                    data_width: int = 32) -> Dict[int, AXIL5Packet]:
    """
...
def get_response_name(resp_code: int) -> str:
    """Get human-readable name for AXIL5 response code."""
...
if __name__ == "__main__":
    print("AXIL5 Packet Utilities Testing")
    print("=" * 40)
    ...
    is_valid, msg = validate_axil5_address_alignment(0x1000, 32)
    print(f"\nAddress validation (0x1000): {is_valid} - {msg}")

    is_valid, msg = validate_axil5_address_alignment(0x1002, 32)
    print(f"Address validation (0x1002): {is_valid} - {msg}")

    print("\nAll AXIL5 packets created successfully using generic field names!")
